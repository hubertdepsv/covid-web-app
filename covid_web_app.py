...
# request data from the esri website
...
raw_json = raw.json()
df = d.DataFrame(raw_json["features"])

# in the case of COVID record we have geographical attributes, numbers and a timestamp
...
# cleaning data
# missing values in the Last_Update and Province_State columns
# Last_Update column values are timestamps in milliseconds
...
# building a dashboard using plotly subplots
# our dashboard will display the daily total of confirmed, recovered and death cases worldwide
# the same totals for each country
# top 10 countries for each total
